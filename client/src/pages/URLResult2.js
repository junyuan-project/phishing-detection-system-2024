...
function URLResult() {
    ...
    //Additional Info
    const [requestsData, setRequestsData] = useState(0);
    const [domainsData, setDomainsData] = useState(0);
    const [subdomainsData, setSubDomainsData] = useState(0);
    const [ipsData, setIpsData] = useState(0);
    const [ipv6Data, setIpv6Data] = useState(0);
    const [cookiesData, setCookiesData] = useState(0);
    ...
    useEffect(() => {
        if (uuid) {
            fetch(`http://localhost:8080/reportedurl/getReportedURLById/${uuid}`)
                .then((response) => {
                    if (!response.ok) {
                        setShowAlert(true);
                ...
                .then((data) => {
                    console.log(data);
                    if (data.status === 429) {
                        setRateLimitMessage("Data Rate Limit");

                    } else if (data) {
                        if (data.data.url_safe === 0) {
                            setSafeData("The URL is safe");
                        } else {
                            setSafeData("The URL have potential malicious");
                        }

                        if (data.data.url_status >= 80 && data.data.url_status <= 100) {
                            setDangerData("Low")
                        } else if (data.data.url_status >= 60 && data.data.url_status <= 79) {
                            setDangerData("Moderate")
                        } else if (data.data.url_status >= 40 && data.data.url_status <= 59) {
                            setDangerData("Considerable")
                        } else if (data.data.url_status >= 20 && data.data.url_status <= 39) {
                            setDangerData("High")
                        } else {
                            setDangerData("Extreme")
                        }

                        setScoreData(data.data.url_score);

                        setURLData(data.data.url);
                        setIpAddData(data.data.url_ip);
                        setDomainNameData(data.data.url_domain);
                        setCountryData(data.data.url_country);

                        setServerData(data.data.url_server);
                        setASNData(data.data.url_asn);
                        setASNNameData(data.data.url_asnname);
                        setSubmitterData(data.data.url_country);

                        const fetchCertificateData = async () => {
                            try {
                                const response2 = await fetch(`http://localhost:8080/reportedurl/getCertificatesById/${data.data.url_certificate_id}`);
                                if (!response2.ok) {
                                    throw new Error('Failed to fetch certificate data');
                                }
                                const data2 = await response2.json();
                                console.log(data2.data)
                                setCertificateData(data2.data); // Set certificateData with fetched data
                                console.log(setCertificateData(data2.data))
                        ...
                        setRequestsData(parseInt(data.data.url_request_length, 10));
                        setDomainsData(parseInt(data.data.url_domain_length, 10));
                        setSubDomainsData(parseInt(data.data.url_subdomain_length, 10));
                        setIpsData(parseInt(data.data.url_ip_length, 10));
                        setIpv6Data(parseInt(data.data.url_ipv6_length, 10));
                        setCookiesData(parseInt(data.data.url_cookies_length, 10));
                    } else {
                        setShowAlert(true);
                                            ...
                                            certificateData.map((certificate, index) => (
                                                <TableRow key={index}>
                                                    <TableCell style={tableCertStyle}>
                                                        <div style={{ width: '30%' }}>{certificate.certificate_name}</div>
                                                        <div style={{ width: '20%' }}>{certificate.certificate_issuer}</div>
                                                        <div>Valid from: {formatDate(certificate.certificate_validFrom)} to {formatDate(certificate.certificate_validTo)}</div>
                                                    </TableCell>
                                                </TableRow>
                                            ))
                                        ) : (
                                            <TableRow>
                                                <TableCell>No certificate data available</TableCell>
                                            </TableRow>
                                        )}
                                    </TableBody>
                                </Table>
